...
static struct light_state_t g_button;
...
static int g_lcd_prev_backlight = -1;
        ...
        struct light_state_t const* state)
{
	...
	if (brightness == 0)
	{
		// When LCD turning off, force button backlights off
		ALOGD("LCD -> off; force buttons off");
		write_int(BUTTON_FILE, 0);
	    write_int(BUTTON_FILE1, 0);
	}
	else if (g_lcd_prev_backlight == 0)
	{
		// When LCD turning on, restore button backlights state
		ALOGD("LCD -> on; restore buttons state");
		write_int(BUTTON_FILE, g_button.color & 0xFF);
    	write_int(BUTTON_FILE1, g_button.color & 0xFF);
	}

	g_lcd_prev_backlight = brightness;
        ...
        struct light_state_t const* state)
{
    ...
    pthread_mutex_lock(&g_lock);
	g_button = *state;
    err = write_int(BUTTON_FILE, g_button.color & 0xFF);
    err = write_int(BUTTON_FILE1, g_button.color & 0xFF);
    pthread_mutex_unlock(&g_lock);
    return err;
}
